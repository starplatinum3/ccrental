...
// extends WithTitles
public class TblData<T> extends JTable implements ActionListener {
    //    JTable jTable;
    //  网点 两个
    T current = null;
...
//, Preserve preserve
   ...
   public void addEmptyRowPushObj(){
        try {
            UiUtil.addEmptyRow(this, current);
...
//        没有用处啊
    ...
    //    T 应该是需要的
    TblData(Object[] titles, Object[][] tblData, DefaultTableModel tabModel,
            List<T> all, ItDao<T> dao) throws BaseException, SQLException {
...
//                i 是 1，有一个 不行，因为 0 1 ，1 是第二个了
                if (i >= TblData.this.all.size()) {
                    return;
                }
                current = TblData.this.all.get(i);
...
//     & Do
    public <T extends CanSetVals> void updateBatch(TblData<T> dataTbl, ItDao<T> dao)
