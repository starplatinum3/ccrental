...
import cn.edu.zucc.personplan.model.Cellable;
...
// extends WithTitles
public class TblDataCellable<T extends Cellable> extends JTable implements ActionListener {
    //    JTable jTable;
    //  网点 两个
    T current = null;
...
//    要使用吗
    public void reloadTableGetCell() {//这是测试数据，需要用实际数替换
//        try {
...
//        }
        ...
        Object[][] tblData = UiUtil.getTblData(all, titles);
//        这几句 可以分离开来
//        Object[][] tblPlanData = new Object[all.size()][titles.length];
//        for (int i = 0; i < all.size(); i++) {
//            for (int j = 0; j < titles.length; j++)
//                tblPlanData[i][j] = all.get(i).getCell(j);
//        }
//        ddd
//    tblPlanTitle obj 列表
//        tabModel.setDataVector(tblPlanData, titles);
        tabModel.setDataVector(tblData, titles);
        this.validate();
        this.repaint();
    }


    TblDataCellable(Object[] titles,
...
//        Object[][] objects = new Object[1][1];
    ...
    public TblDataCellable(Object[] titles,
...
//        没有用处啊
    ...
    //    T 应该是需要的
    TblDataCellable(Object[] titles, Object[][] tblData, DefaultTableModel tabModel,
                    List<T> all, ItDao<T> dao) throws BaseException, SQLException {
...
//                这样可以的
//                i 是 1，有一个 不行，因为 0 1 ，1 是第二个了
                if (i >= TblDataCellable.this.all.size()) {
                    return;
                }
                current = TblDataCellable.this.all.get(i);
...
//     & Do
    public <T extends CanSetVals & Cellable> void updateBatch(TblDataCellable<T> dataTbl, ItDao<T> dao)
