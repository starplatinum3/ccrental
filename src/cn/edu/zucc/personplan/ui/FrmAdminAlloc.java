...
public class FrmAdminAlloc extends JFrame implements ActionListener {
    ...
    private TblDataDo<NetInfo> tddInNetInfo;
    private TblDataDo<NetInfo> tddOutNetInfo;
...
//    String[] titles = new String[]{"车", "报废信息", "车","报废信息"};
    String[] titles = new String[]{"车辆调拨", "车子", "调出网点","调入网点"};

    public FrmAdminAlloc() throws BaseException, SQLException {

        this.setTitle("管理车辆调拨");
...
//        setSize(1000,800);
        ...
        this.tddInNetInfo = new TblDataDo<>(NetInfo.tableTitles, CcCarUtil.netInfoDao);
        this.tddOutNetInfo = new TblDataDo<>(NetInfo.tableTitles, CcCarUtil.netInfoDao);
//        写在前面的就能显示
...
//        tblDataList.add(tddScrap);
        ...
        tblDataList.add(tddCarInfo);
        tblDataList.add(tddOutNetInfo);

        tblDataList.add(tddInNetInfo);
...
//        tddCarInfo
        tdAllocation.addMouseListener(new MouseAdapter() {

            @Override
            public void mouseClicked(MouseEvent e) {

                int i = tdAllocation.getSelectedRow();
//                这个知道是自己吗
                if (i < 0) {
                    return;
                }
                ...
                CarInfo carInfo = new CarInfo();
                carInfo.setCarId(tdAllocation.current.getCarId());

                NetInfo netInfoIn = new NetInfo();
                netInfoIn.setNetId(tdAllocation.current.getNetInId());

                NetInfo netInfoOut = new NetInfo();
                netInfoOut.setNetId(tdAllocation.current.getNetOutId());
...
//                    tddCarInfo.reloadDataAndRepaint(carInfo);
                    tddCarInfo.reloadDataAndRepaint(carInfo);
                    tddInNetInfo.reloadDataAndRepaint(false,netInfoIn);
                    tddOutNetInfo.reloadDataAndRepaint(false,netInfoOut);
