...
public class FrmEmpCoupon extends JFrame implements ActionListener {
...
//    private JButton add = new JButton("点单");
    ...
    private JButton btnAddNewCoupon = new JButton("新增优惠券coup");
    private JButton btnAddNewPromoComplete = new JButton("提交优惠券");
    private JButton btnCommitCoupon = new JButton("提交优惠券coup");
    ...
    private JTextField edtDeduc = new JTextField(20);
...
//                ex.printStackTrace();
//            }
        } else if (e.getSource() == this.btnCommitCoupon) {
//            btnAddNewPromoCompleteEvent();
            try {
                btnCommitCouponEvent();
    ...
    void btnCommitCouponEvent() throws BaseException, SQLException {

        String contentText = edtContent.getText();
        String deducText = edtDeduc.getText();
...
//            }
        ...
        if (StringUtils.isNone(deducText)) {
            UiUtil.showError("请输入减少多少钱");
            return;
        }
        BigDecimal deduc = new BigDecimal(deducText);
        if (deduc.compareTo(BigDecimal.ZERO) < 0) {
            UiUtil.showError("你不能让他倒贴钱啊..");
            return;
        }
...
//                JdbcUtil.update(promotion,promotion.getPromId(),"prom_id");
        ...
        coupon1.setNetId(CcCarUtil.currentEmp.getNetId());
        tdCoupon.reloadDataAndRepaint(coupon1);
...
//        dataTablePromotionReload();
    ...
    void reloadCoupon() throws BaseException, SQLException {
        Coupon coupon1 = new Coupon();
        coupon1.setNetId(CcCarUtil.currentEmp.getNetId());
        tdCoupon.reloadDataAndRepaint(coupon1);
    }
...
//            }
        ...
        if (StringUtils.isNone(discountText) || StringUtils.isNone(promQuantityText)) {
            UiUtil.showError("填入有空字符创");
            return;
        }
...
//    FrmUserOrder frmUserOrder=new FrmUserOrder("用户查看订单",orde);
    ...
    public FrmEmpCoupon() throws BaseException, SQLException {
//            Bus.fUserGo = this;
        this.setTitle("管理界面");
...
//        inputPanel.add(edtPromQuantity);
        ...
        inputPanel.add(lblContent);
        inputPanel.add(edtContent);
        inputPanel.add(new JLabel("减少多少钱"));
        inputPanel.add(edtDeduc);
...
//                TblData<CarInfo> tableCarInfo = FrmProm.this.dataTableCarInfo;
                TblData<CarType> carTypeTblData = FrmEmpCoupon.this.tblCarType;
...
//        btnList.add(this.btnAddNewPromoComplete);
        btnList.add(this.btnCommitCoupon);
...
//                tblDataPromotion, tabModelPromotion, allPromotion, CcCarUtil.promotionDao);
        ...
        tdCoupon = new TblData<>(Coupon.tableTitles,
                CcCarUtil.couponDao);
